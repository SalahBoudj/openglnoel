...
int Application::run()
{
	...
	const auto viewMatrix = tbc.getViewMatrix(0.0f,0.0f,0.0f);



	//Pour la trackball caméra
	if (glfwGetMouseButton(m_GLFWHandle.window(), GLFW_MOUSE_BUTTON_LEFT) && !m_LeftButtonPressed) {
       	 	m_LeftButtonPressed = true;
		std::cout <<"OOH" << std::endl;
       		glfwGetCursorPos(m_GLFWHandle.window(), &m_LastCursorPosition.x, &m_LastCursorPosition.y);
    	}
   	else if (!glfwGetMouseButton(m_GLFWHandle.window(), GLFW_MOUSE_BUTTON_LEFT) && m_LeftButtonPressed) {
  	...
  	if (m_LeftButtonPressed) {
        	...
        	glfwGetCursorPos(m_GLFWHandle.window(), &cursorPosition.x, &cursorPosition.y);

			if(cursorPosition.x > m_LastCursorPosition.x){
				mouse_x = mouse_x + 1.;
				tbc.rotateUp( mouse_x );
			}
			if(cursorPosition.x < m_LastCursorPosition.x){
				mouse_x = mouse_x - 1.;
				tbc.rotateUp( mouse_x );
			}
			if(cursorPosition.y > m_LastCursorPosition.y){
				mouse_y = mouse_y + 1.;
				tbc.rotateLeft( mouse_y );
			}
			if(cursorPosition.y < m_LastCursorPosition.y){
				mouse_y = mouse_y - 1.;
				tbc.rotateLeft( mouse_y );
